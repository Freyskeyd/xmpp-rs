...
impl XmppParser {
    pub fn source<'a>(&'a self) -> &'a Buffer{
        self.parser.source()
    }
    ...
    pub fn next_event(&mut self) -> Option<Event> {
        ...
        loop {
            trace!("next_event");
            // trace!("     -> pos {:?}", self.parser.source().position());
            ...
            match self.parser.next() {
                    ...
                    match xml_event {
                        ...
                        XmlEvent::StartElement { name, namespace, attributes} => {
                            if name.local_name == "features" && name.namespace_ref() == Some(ns::STREAM) {
                                ...
                                match Element::from_start_element(name, attributes, namespace, None, &mut self.parser) {
                                    Ok(e) => {
                                        let e = StreamFeatures::from_element(e);
                                        return Some(e.to_event());
                                    },
                                    Err(_) => {}
                                }
                                ...
                                match Element::from_start_element(name, attributes, namespace, None, &mut self.parser) {
                                    Ok(e) => {
                                        let e = GenericIq::from_element(e);
                                        match e {
                ...
                Err(ref e) if e.kind().eq(&XmlErrorKind::Syntax(Cow::from("Invalid processing instruction: <?xml"))) => continue,
                Err(e) => {
                    trace!("Error {:?}", e);
                    break;
                }
